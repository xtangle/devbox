...
###
# Note: This script is depreated.
#
# The purpose of this script originally was to accommodate for the default installation of Pop!_OS, but since
# the base box has been updated to have only one primary partition and a swapfile is used instead,
# it is no longer needed.
#
# ---
#
# Run this if the VM hard disk is configured with 2 partitions: /dev/sda1 and /dev/sda2, where
# /dev/sda1 is your primary data partition, and /dev/sda2 is your dedicated *encrypted* swap partition.
#
# This script allocates a specific size for swap partition (16GB), moves it
# to the end of the disk sector, and allocates the rest of the disk to the data partition.
###
...
if [[ -f "${continue_from_prev_file}" ]]; then
  ...
  # create new crypttab configuration
  sudo mkswap /dev/sda2
  sudo swapon /dev/sda2
  sudo yes | sudo ecryptfs-setup-swap || true # this is known to fail
